...
###############################################
# ACX_FORTRAN_ISNAN checks for the presence of the isnan intrinsics
# --------------------------
AC_DEFUN([ACX_FORTRAN_ISNAN], [
    ...
    [
      AC_MSG_RESULT(yes)
      AC_DEFINE(HAVE_FORTRAN_ISNAN, 1,
                [Define if the compiler provides the isnan instrinsic])
    ],
    [
      AC_MSG_RESULT(no)
    ])
