...
!
! parameters:
! real*8 a(:,:) (in):  matrix to be collected
...
!
! parameters:
! integer a(:,:) (in):  matrix to be collected
...
if (rank .eq. root) then
  ...
  k = size(b)
  if (i .ne. k) then
     print *,'process ',rank,' has a problem in matrix_distr_sendmat_integer: '
     print *,'number of words to receive:',k
     print *,'root expects:              ',i
     call MPI_Abort(MPI_COMM_WORLD,1,ierror)
  endif
...
if (rank .eq. root) then
  ...
  if (i .ne. k) then
     print *,'process ',rank,' has a problem in matrix_coll_recvmat_real8: '
     print *,'number of words to send:',k
     print *,'root expects:           ',i
     do i=1,procs
       print *,i-1,isleft(i),isright(i),istop(i),isbot(i)
     enddo
     print *,isleftl,isrightl,istopl,isbotl
     call MPI_Abort(MPI_COMM_WORLD,1,ierror)
  endif
...
#if 0
subroutine create_cart_comm(m,n,comm,root,cartcomm,rank,loci,locj, &
                             left,right,bot,top,newroot)
! Create cartesian communicator, and determines the location
! of the new root
  use mpi
  implicit none
  integer, intent(in)   :: m,n,comm,root
  integer, intent(out)  :: cartcomm,rank,left,right,bot,top,newroot
  integer, intent(out)  :: loci, locj

  integer               :: ierror, isize, i
  integer, dimension(2) :: dims, periods
  ...
  call MPI_Cart_create(comm, 2, dims, periods, 1, cartcomm, ierror)
  call MPI_Comm_rank(comm, rank, ierror)
  call MPI_Cart_coords(cartcomm, rank, 2, dims, ierror)
  loci = dims(1)
  locj = dims(2)

  call MPI_Cart_shift(cartcomm, 0, 1, left, right, ierror)
  call MPI_Cart_shift(cartcomm, 1, 1, bot,  top,   ierror)

  call MPI_Comm_rank(cartcomm, newroot, ierror)
  call MPI_Bcast(newroot, 1, MPI_INTEGER, root, comm)
