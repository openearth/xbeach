...
Lmix = 2*s.R.*par.Tp./max(0.01,(par.rho.*s.c.*s.H));
Lmix = min(Lmix,h+0.5*s.H);
...
% plot(xw,s.ceqgm*rhos,'k--','LineWidth',1.5);
xlabel('x [m]'); ylabel('c [g/l]'); axis([41 max(max(xw)) 0 25]);
...
xlabel('x [m]'); ylabel('k [m^2/s^2]'); axis([41 max(max(xw)) 0 3.5]);
print turbulence.png -dpng;

% mixing length
figure(15); set(gca,'FontSize',16);
plot(xw(:,2),mean(Lmix./h,2),'k','LineWidth',1.5); hold on; plot(xw(:,2),mean(Lmix2./h,2),'k--','LineWidth',1.5); plot(xw(:,2),mean(0.9*s.H./h,2),'k:','LineWidth',1.5);
...
plot(xw(:,2),mean(s.BR,2),'k','LineWidth',1.5); hold on; 
plot(xw(:,2),mean(tan(asin(s.BR)),2),'k--','LineWidth',1.5);
xlabel('x [m]'); ylabel('\beta & \partial\eta/\partialx [-]'); axis([41 max(max(xw)) 0 1.2*max(mean(tan(asin(s.BR)),2))]);
