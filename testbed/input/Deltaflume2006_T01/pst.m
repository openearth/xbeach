...
xlabel('x [m]'); ylabel('c [g/l] & c_{eq} [g/l]'); axis([0 max(max(xw)) 0 2*rhos*max(max(s.Cm))]);
...
plot([0.1 25],[0.1 25],'k','LineWidth',1.5);
plot([0.1 25],0.5*[0.1 25],'k--','LineWidth',1.5);
plot([0.1 25],2*[0.1 25],'k--','LineWidth',1.5);
xlabel('c_{measured} [g/l]'); ylabel('c_{simulated} [g/l]'); axis image
axis([0.1 25 0.1 25]); grid on;
