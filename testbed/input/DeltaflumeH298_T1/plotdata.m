...
% load hard layer
hardlayer = load('hardlayer_T1.dep');
...
plot(xw(1:indh,2),zhard(1:indh),'color',[0.6 0.6 0.6],'LineWidth',9);
...
figure; set(gca,'FontSize',16);
plot(xw(:,2),mean(s.cc*rhos,2),'k','LineWidth',2);
axis([150 210 0 5]);
...
plot(xw,mean(s.Sdzbtot,2)-s.Sdzbtot(1)+mean(s.Su(1,:),2),'k-','LineWidth',1.5);
plot([xw(194,2) xw(194,2)],[-5.0e-5 1e-5],'r--','LineWidth',1.5);
axis([150 210 -5.0e-5 1e-5]);
subplot(212);
plot(xw(:,2),s.zb(:,1),'k-','LineWidth',1.5); hold on; 
for i = 1:length(z)
...
plot([0 210],[0 0],'k:');
plot([xw(194,2) xw(194,2)],[-2 2.5],'r--','LineWidth',1.5);
axis([150 210 -2 2.5]); xlabel('x [m]'); ylabel('z_b [m]');
