...
load hardlayer_T1.dep
load bed_T1.dep

dzg = 0.1;
dx = 1;
x = [0:1:length(bed_T1)-1]*dx;
...
zlm = repmat(zl,length(x),1)' + repmat(bed_T1(1,:),20,1);
xm = repmat(x,20,1);


figure; 
subplot(211);
plot(x,bed_T1(1,:),'r-','LineWidth',3); hold on; pcolor(xm,zlm,gdist1); colorbar;
axis([60 100 -4 3]);
subplot(212);
plot(x,bed_T1(1,:),'r-','LineWidth',3); hold on; pcolor(xm,zlm,gdist2); colorbar;
axis([60 100 -4 3]);
