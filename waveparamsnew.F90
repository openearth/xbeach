...
!   real*8,dimension(:),pointer            :: danggen    ! Representative integration angle for Srep to Sf, per offshore grid point                                                        
   ...
   character(256)                         :: Efilename,qfilename
endtype
...
integer,dimension(:),allocatable,save         :: n_index_loc     ! y-index locations of all input spectra
integer,save                                  :: nspectra        ! number of input spectrs
...
integer,save                                  :: bccount         ! number of times boundary conditions have been generated
...
integer                     :: iloc
integer                     :: fidelist,fidqlist ! file identifiers for ebcflist and qbcflist
...
endsubroutine spectral_wave_bc
    ...
    ! --------------------------------------------------------------
    subroutine interpolate_spectrum(specin,specinterp,par)
    ...
    specinterp%nf = nfint
    specinterp%df = 2.d0/(nfint-1)       ! this give a range of 0-2Hz, with 1/200Hz resolution
    do i=1,nfint
    ...
    call frange(par,combspec%Sf,ind1,ind2)
    ...
    do i=1,ntaper
       wp%taper(indend+1-i) = tanh(5.d0*i/ntaper)
    enddo
